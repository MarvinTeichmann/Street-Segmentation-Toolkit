...
data_prefix = "/home/mteichmann/workarea/DATA/MedizinInformatik/"
data_txt_path = "Segmentation_Rigid_Training/data.txt"
data_txt_path = os.path.join(data_prefix, data_txt_path)
label_txt_path = "Segmentation_Rigid_Training/label.txt"
label_txt_path = os.path.join(data_prefix, label_txt_path)

training_ratio = 1

data_paths =  [i.strip() for i in open(data_txt_path, "r")]
label_paths = [i.strip() for i in open(label_txt_path, "r")]

assert len(data_paths) == len(label_paths), \
    "length of data.txt does not match length of label.txt"

# Make a split and 

zipped = list(zip(data_paths, label_paths))
